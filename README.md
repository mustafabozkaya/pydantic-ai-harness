...
The most-discussed HN story (>=100 points) across the top, best, and show feeds is "Google Chrome silently installs a 4 GB AI model on your device without consent" by user john-doe (thatprivacyguy.com) -- 1,715 points and 1,120 comments, appearing on the top feed. The article alleges that Chrome auto-downloads a 4 GB weights.bin file (the Gemini Nano model) into an OptGuideOnDeviceModel folder on qualifying machines with no opt-in, re-fetching it if deleted, which the author argues may violate EU consent law and wastes meaningful network energy. The HN thread is sharply divided: top commenters like crazygringo and doginasuit push back that "consent" framing is misguided since users already consented to autoupdates (analogous to a bundled spellcheck dictionary), and TheServitor dismisses the carbon-cost angle as a stretch; meanwhile sysadmins (davb) flag real operational pain -- 4 GB x thousands of student profiles on NFS or Windows AppData -- and others (jacquesm, toyg, tdeck) attack auto-update abuse and the lack of even a token opt-out dialog, with newsoftheday sharing a Linux removal recipe via chrome://flags (disable "optimization guide on device" and "Prompt API for Gemini Nano"). The submitter john-doe is a moderately established 2020-era account (karma 2,941, 301 submissions) whose other recent posts barely registered -- this story is by far his breakout hit. The story has spread well beyond HN: Tom's Hardware, The Register, TechPowerUp, Yahoo Tech, and OSTechnix have all covered it within ~2 days, generally citing researcher Pat Hanff, emphasizing the missing consent flow, potential EU law implications, and the file's stubborn re-download behavior.
"""
```
