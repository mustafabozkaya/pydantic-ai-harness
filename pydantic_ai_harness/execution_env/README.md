...
> **Status: in development.** `read_file` is complete and tested; `write_file`/edit, explore (`ls`/`glob`/`grep`), and `shell` are in progress. `LocalEnvironment` works today; `DockerEnvironment` is planned. This README describes the intended shape.
